...
from car import move, check_car_collision, MAX_STEER, WB, plot_car, BUBBLE_R, CarInfo
...
def main():
    ...
    lane_width = 6.0
    park_slot_width = 3.0
    park_slot_length = 5.0
    
    # Safety tresholds
    car_back_safe_dis = 0.5

    # Obstacle points: parking scene
    ox, oy = [], []
    for i in np.linspace(-15+park_slot_width/2, 15+park_slot_width/2, 60):    #upper lane
        ox.append(i)
        oy.append(6.0)
    for i in np.linspace(-15+park_slot_width/2, 0, 30):    #lower lane left
        ox.append(i)
        oy.append(0.0)
    for i in np.linspace(park_slot_width, 15+park_slot_width/2, 30):    #lower lane right
        ox.append(i)
        oy.append(0.0)
    for i in np.linspace(0, park_slot_width, 20):    #slot buttom
        ox.append(i)
        oy.append(-park_slot_length)
    for i in np.linspace( -park_slot_length, 0, 20):    #slot left, right edges
        ox.append(0)
        oy.append(i)
        ox.append(park_slot_width)
        oy.append(i)

    # Set Initial parameters
    start = [3, 1.5, np.deg2rad(0.0)]
    goal = [park_slot_width/2, -park_slot_length + car_back_safe_dis + carInfo.rear_to_back_end, np.deg2rad(90.0)]
